...
const Checkout = () => {
  ...
  const getAddresses = async (token) => {
    ...
    try {
      const response = await axios.get(`${config.endpoint}/users/addresses`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const addAddress = async (token, newAddress) => {
    try {
      ...
      const response = await axios.post(`${config.endpoint}/users/addresses`, {address: newAddress.value}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
      if (response.data.length === 1) {
        setAddresses({ ...addresses, all: response.data, selected: response.data[0]._id});
      } else {
        setAddresses({ ...addresses, all: response.data});
      }
  ...
  const deleteAddress = async (token, addressId) => {
    try {
      const response = await axios.delete(`${config.endpoint}/users/addresses/${addressId}`, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
  ...
  const performCheckout = async (token, items, addresses) => {
    ...
    try {
      await axios.put(`${config.endpoint}/cart/checkout`, {"addressId": addresses.selected}, {
        headers: {
          Authorization: `Bearer ${token}`,
        },
      });
            ...
            <Box className="shipping-container" minHeight="100vh">
              ...
              <Box>
                {addresses.all? addresses.all.map(address => (
                    <Box className={addresses.selected === address._id? "address-item selected": "address-item not-selected"} key={address._id} onClick={() => {
